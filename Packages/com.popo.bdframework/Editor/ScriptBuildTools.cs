...
public class ScriptBuildTools
{
    ...
    static public string ProjectRoot
    {
    ...
    {
        get { return ProjectRoot + "/Library"; }
    }

    private static Dictionary<int, string> csFilesMap;
    private static string DLLPATH { get; set; } = "Hotfix/hotfix.dll";
    ...
    static public void BuildDll(string outPath, RuntimePlatform platform, BuildMode mode, bool isShowTips = true)
    {
        ...
        //准备输出环境
        var _outPath = Path.Combine(outPath, GetPlatformPath());
        try
        {
        ...
        ParseCsprojFile(gameLogicCsproj, new List<string>(), ref csFileList, ref dllFileList);
    ...
    {
        var projpath = ProjectRoot + "/" + projName;
        ...
        foreach (var csproj in csprojList)
        {
            ...
            //
            var gendll = Library + "/ScriptAssemblies/" + csproj.Replace(".csproj", ".dll");
